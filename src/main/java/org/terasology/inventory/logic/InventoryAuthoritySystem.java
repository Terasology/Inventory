...
public class InventoryAuthoritySystem extends BaseComponentSystem implements InventoryManager {
    ...
    public void removeItem(RemoveItemAction event, EntityRef entity) {
        final EntityRef result = removeItemInternal(entity, event.getInstigator(), event.getItems(),
                event.isDestroyRemoved(), event.getCount());
        if (result != null) {
    ...
    private EntityRef removeItemFromSlots(EntityRef instigator, boolean destroyRemoved, EntityRef entity,
                                          List<Integer> slotsWithItem, int toRemove) {
        int shrinkSlotNo = -1;
    ...
    private EntityRef removeItemInternal(EntityRef inventory, EntityRef instigator, List<EntityRef> items,
                                         boolean destroyRemoved, Integer count) {
