...
public class ItemCommands extends BaseComponentSystem {
    ...
    public String remove(
        ...
        final Set<ResourceUrn> matchingShapeUrns = (shapeUri == null ? null : assetManager.resolve(shapeUri,
                BlockShape.class));
        ...
        if (matchingUrns.size() == 1) {
            if (isBlock) {
                BlockFamily blockFamily = assetManager.getAsset(matchingUrns.iterator().next(),
                        BlockFamilyDefinition.class).map(def -> {
                    if (def.isFreeform()) {
                        ResourceUrn shapeUrn = (matchingShapeUrns == null ? new ResourceUrn("engine:cube") :
                                matchingShapeUrns.iterator().next());
    ...
    private int removeObjectFromSlot(EntityRef slot, EntityRef playerEntity, boolean isStackable, String displayName,
                                     int quantity) {
        ...
        if (isStackable) {
            ...
            if (itemComponent != null) {
                if (quantityLeft >= itemComponent.stackCount) {
                    result = inventoryManager.removeItem(playerEntity, EntityRef.NULL, slot, true,
