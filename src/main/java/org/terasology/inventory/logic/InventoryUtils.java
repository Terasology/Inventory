...
public final class InventoryUtils {
    ...
    private static boolean validateMove(EntityRef instigator, EntityRef from, int slotFrom, EntityRef to, int slotTo) {
        ...
        if (itemFrom.exists()) {
            BeforeItemRemovedFromInventory removeFrom = new BeforeItemRemovedFromInventory(instigator, itemFrom,
                    slotFrom);
        ...
        if (itemFrom.exists()) {
            ...
            to.send(putTo);
            return !putTo.isConsumed();
        }

        return true;
    }

    private static boolean validateMoveAmount(EntityRef instigator, EntityRef from, int slotFrom, EntityRef to,
                                              int slotTo, int amount) {
        ...
        if (itemTo == null) {
            BeforeItemPutInInventory putTo = new BeforeItemPutInInventory(instigator, InventoryUtils.getItemAt(from,
                    slotFrom), slotTo);
            to.send(putTo);
            return !putTo.isConsumed();
        }
     ...
     */
    private static boolean moveToFreeSlots(EntityRef instigator, EntityRef from, int slotFrom, EntityRef to,
    ...
    static boolean moveItemToSlots(EntityRef instigator, EntityRef from, int fromSlot, EntityRef to,
                                   List<Integer> toSlots) {
    ...
    static boolean moveItemAmount(EntityRef instigator, EntityRef from, int slotFrom, EntityRef to, int slotTo,
                                  int amount) {
