...
public class InventoryGrid extends CoreWidget {
    private final List<InventoryCell> cells = Lists.newArrayList();
    private final InteractionListener interactionListener = new BaseInteractionListener() {
        ...
        public boolean onMouseClick(NUIMouseClickEvent event) {
            ...
            return mouseButton == MouseInput.MOUSE_LEFT;
        }
    };
    /**
     * Defines the maximum amount of cells displayed in a single row of InventoryGrid.
     * <p>
     * Example: If an inventory has 20 slots and maxHorizontalCells is set to 5, the grid will have 4 rows of cells
     * (grid size will be 5x4).
     */
    @LayoutConfig
     ...
     * <p>
     * Example: If an inventory has 20 slots and cellOffset is set to 5, 15 cells will be drawn starting from slot no. 5
     * (starting from zero). Other slots will be not accessible by this InventoryGrid.
     */
    @LayoutConfig
    private Binding<Integer> cellOffset = new DefaultBinding<>(0);
    /**
     * Defines the maximum amount of cells in an InventoryGrid. Used together with cellOffset, it allows developers to
     * provide access to parts of an entity's inventory.
     * <p>
     * Example: If an inventory has 20 slots, maxCellCount is set to 10 and cellOffset to 5, there will be 10 cells
     * drawn, starting from slot no. 5 and ending at cell no. 14 (starting from zero).
     */
    @LayoutConfig
     ...
     *
     * @param binding A Binding of the EntityRef type referring to the entity whose inventory will be
     *         displayed.
     */
