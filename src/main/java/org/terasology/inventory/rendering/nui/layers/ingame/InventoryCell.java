...
public class InventoryCell extends ItemCell {
    ...
    private final InteractionListener interactionListener = new BaseInteractionListener() {
        ...
        public boolean onMouseWheel(NUIMouseWheelEvent event) {
            ...
            int amount =
                    (keyboard.isKeyDown(Keyboard.KeyId.RIGHT_CTRL) || keyboard.isKeyDown(Keyboard.KeyId.LEFT_CTRL)) ?
                            2 : 1;
    ...
    private void giveAmount(int amount) {
        EntityRef characterEntity = CoreRegistry.get(LocalPlayer.class).getCharacterEntity();
        InventoryManager inventoryManager = CoreRegistry.get(InventoryManager.class);
        inventoryManager.moveItem(getTargetInventory(), characterEntity, getTargetSlot(), getTransferEntity(), 0,
                amount);
    }
    ...
    private void takeAmount(int amount) {
        EntityRef characterEntity = CoreRegistry.get(LocalPlayer.class).getCharacterEntity();
        InventoryManager inventoryManager = CoreRegistry.get(InventoryManager.class);
        inventoryManager.moveItem(getTransferEntity(), characterEntity, 0, getTargetInventory(), getTargetSlot(),
                amount);
    }
    ...
    private void moveItemSmartly() {
        ...
        CoreRegistry.get(InventoryManager.class).moveItemToSlots(getTransferEntity(), fromEntity, fromSlot,
                targetEntity, toSlots);
    }
