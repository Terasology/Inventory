...
public class StartingInventorySystemTest {
    ...
    public void giveSingleBlock() {
        EntityRef item = setupItem(1, 10, "blockFamilyA", 1L);
        ...
        when(blockItemFactory.newInstance(blockFamily)).thenReturn(item);
        ...
        // ensure that the starting inventory component was removed after adding the items
        assertFalse(entityRef.hasComponent(StartingInventoryComponent.class));
        // inventory after sending the event
        final List<EntityRef> itemSlots = entityRef.getComponent(InventoryComponent.class).itemSlots;
        assertEquals(1, itemSlots.stream().filter(elem -> elem != EntityRef.NULL).count());
        assertEquals(item, itemSlots.get(0));
    }

    @Test
    public void giveBlockStack() {
        EntityRef block = setupItem(1, 99, "blockFamilyA", 1L);
        ...
        // Setup the factory to return the right instances
        when(blockItemFactory.newInstance(blockFamily)).thenReturn(block);
        ...
        // inventory after sending the event
        final List<EntityRef> itemSlots = entityRef.getComponent(InventoryComponent.class).itemSlots;
        assertEquals(block, itemSlots.get(0));
        assertEquals(3, itemSlots.get(0).getComponent(ItemComponent.class).stackCount);
        assertEquals(1, itemSlots.stream().filter(elem -> elem != EntityRef.NULL).count());
    }
//
//    @Test
...
//    }
    ...
    //=======================================================================================
    private EntityRef setupItem(int amount,
                                int maxStackSize,
                                String stackId,
                                long id) {
        ItemComponent itemComp = new ItemComponent();
        ...
        itemComp.stackCount = (byte) amount;
        itemComp.maxStackSize = (byte) maxStackSize;
        itemComp.stackId = stackId;
        when(item.exists()).thenReturn(true);
        when(item.getComponent(ItemComponent.class)).thenReturn(itemComp);
        when(item.iterateComponents()).thenReturn(Lists.newArrayList());
